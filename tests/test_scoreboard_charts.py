...
def _all_svgs():
    return [
        charts.stat_band_chart([("19", "popular repos"), ("66,645", "commits"),
                                ("4%", "AI-built"), ("99.7%", "backed")]),
...
def test_stat_band_renders_caller_numbers():
    svg = charts.stat_band_chart([("19", "popular repos"), ("66,645", "commits read")])
    assert svg.startswith("<svg")
    assert "19" in svg and "66,645" in svg
    assert "popular repos" in svg and "commits read" in svg


def test_empty_inputs_render_nothing():
    assert charts.stat_band_chart([]) == ""
