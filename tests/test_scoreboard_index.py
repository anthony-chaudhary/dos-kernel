...
def test_front_door_has_the_hero_stat_band():
    readme, assets = idx.build(stamp="2026-06-16")
    assert assets[idx.ASSET_STAT_BAND].startswith("<svg")
    assert f"]({idx.ASSET_STAT_BAND})" in readme  # embedded at the top


def test_who_builds_whose_repo_is_derived_from_markers():
    paths = idx.rollup.find_sweeps(idx.SCOREBOARD)
    rows = idx.leaderboard_rows(paths)
    wb = idx.who_builds_facts(rows)
    assert wb["top_agent"]
    assert 0 < wb["top_count"] <= wb["total"]
    readme, _ = idx.build(stamp="2026-06-16")
    assert "## Who builds whose repo" in readme
    assert wb["dogfood"], "expected at least one vendor-dogfood exception"


def test_who_builds_names_only_published_repos():
    # docs/311 §2: never name an unpublished repo. Every repo named in the
    # who-builds facts must be one of the rows (all published, named pages).
    paths = idx.rollup.find_sweeps(idx.SCOREBOARD)
    rows = idx.leaderboard_rows(paths)
    named_rows = {r["full"] for r in rows}
    wb = idx.who_builds_facts(rows)
    for repo, _agent in wb["dogfood"]:
        assert repo in named_rows
    for repo, _t, _c in wb["rivals"]:
        assert repo in named_rows
