...
# ---------------------------------------------------------------------------
# The `[improve]` config seam (issue #37) — modelled on `dos.cooldown`.
#
...
def policy_from_table(table: dict, *, base: "ImprovePolicy" = DEFAULT_POLICY) -> "ImprovePolicy":
    """Build a `ImprovePolicy` from a parsed `[improve]` TOML table. PURE.
    ...
    if not isinstance(table, dict):
        raise ValueError(f"[improve] must be a table, got {type(table).__name__}")
    known = {"max_consecutive_reverts", "efficiency_floor", "min_tokens_for_efficiency"}
    unknown = set(table) - known
    if unknown:
        raise ValueError(
            f"[improve] has unknown key(s) {sorted(unknown)}; "
...
def load_from_toml(path, *, base: "ImprovePolicy" = DEFAULT_POLICY) -> "ImprovePolicy":
    """Build a `ImprovePolicy` from a `dos.toml`'s `[improve]` table.

    Returns ``base`` unchanged (the SAME object) when the file is absent, has no
    `[improve]` table, or it is empty. A present-but-malformed table raises.
