...
def _gather_attest_readback(args: argparse.Namespace, cfg) -> "tuple[object, str, str] | None":
    ...
    _surfaces = sum(1 for s in (accept_cmd, (before or after), content, ledger) if s)
    if _surfaces > 1:
        return None, "", "give exactly ONE of --accept-cmd / --before+--after / --content / --ledger"
    if content:
    ...
    if ledger:
        # The provider-ledger rung (docs/261 THIRD_PARTY): '<ref>@<ledger>#<expect>'. The
        # driver looks the agent's claimed reference up in a different-principal ledger
        # (a host-captured provider dump committed to git, or a host-wired `source:`
        # validator) and checks the claimed fields match — at the rung the record's and
        # expectation's provenance earn (capped at min(record, expect)).
        provider_ledger = _load_witness_driver("provider_ledger")
        source = provider_ledger.ProviderLedgerEvidenceSource(cwd=str(cfg.paths.root))
        facts = gather_evidence(source, ledger, cfg)
        return witness_effect(claim, [facts]), ledger, ""
    if accept_cmd:
        os_acceptance = _load_witness_driver("os_acceptance")
        source = os_acceptance.OsAcceptanceEvidenceSource(cwd=str(cfg.paths.root))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pat.add_argument("--ledger", default=None, metavar="REF@LEDGER#EXPECT",
                     help="witness via a provider-ledger lookup (docs/261 THIRD_PARTY): "
                          "look the agent's claimed reference '<ref>' up in a "
                          "different-principal ledger '<ledger>' (<path>@<sha> / "
                          "source:<validator>:<subj>) and check '<expect>' (inline:k=v,k2=v2 "
                          "/ source:<validator>:<subj>). Rung = min(record, expect); a "
                          "forgeable expectation is refused belief. Mutually exclusive")
