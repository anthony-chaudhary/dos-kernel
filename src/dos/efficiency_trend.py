...
# ---------------------------------------------------------------------------
# The `[efficiency_trend]` config seam (issue #37) — modelled on `dos.cooldown`.
#
...
def policy_from_table(table: dict, *, base: "TrendPolicy" = DEFAULT_POLICY) -> "TrendPolicy":
    """Build a `TrendPolicy` from a parsed `[efficiency_trend]` TOML table. PURE.
    ...
    if not isinstance(table, dict):
        raise ValueError(f"[efficiency_trend] must be a table, got {type(table).__name__}")
    known = {"min_samples", "tolerance"}
    unknown = set(table) - known
    if unknown:
        raise ValueError(
            f"[efficiency_trend] has unknown key(s) {sorted(unknown)}; "
...
def load_from_toml(path, *, base: "TrendPolicy" = DEFAULT_POLICY) -> "TrendPolicy":
    """Build a `TrendPolicy` from a `dos.toml`'s `[efficiency_trend]` table.

    Returns ``base`` unchanged (the SAME object) when the file is absent, has no
    `[efficiency_trend]` table, or it is empty. A present-but-malformed table raises.
