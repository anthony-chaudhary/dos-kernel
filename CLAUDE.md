...
> caught-lie demo). Install is **from this clone** (`pip install -e .` — `dos-kernel`
...
- **The generic skill pack** (SKP, `docs/74_*`) — five domain-free `SKILL.md`
  screenplays under `src/dos/skills/`, shipped as package-DATA (not code — nothing
...
python -m pytest -q         # the full kernel suite (must stay green; ~3,900 tests, ~3–4 min — foreground, wait for the verdict)
...
- **SKP** — Skill Pack (docs/74: the five domain-free generic `SKILL.md` screenplays
