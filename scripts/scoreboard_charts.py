...
# ---------------------------------------------------------------------------
# chart 0 — the hero stat band: the 4-5 numbers a skimmer should leave with.
# ---------------------------------------------------------------------------


def stat_band_chart(stats: list[tuple[str, str]], *, width: int = 720) -> str:
    """A row of big-number / small-label cells — the headline figures up top.

    `stats` is a list of (big, label) pairs already formatted by the caller
    (e.g. ("19", "popular repos"), ("66,645", "commits read")). Pure; the
    numbers come from the caller. Renders 2..6 cells evenly across the width,
    each a big value over a muted caption. Empty → "".
    """
    cells = [(str(b), str(l)) for b, l in stats if str(b)]
    if not cells:
        return ""
    n = len(cells)
    height = 96
    ...
    out = [_svg_open(width, height,
                     ...
                     desc="Headline figures across the published set.")]
    for i, (big, label) in enumerate(cells):
        cx = 1 + cell_w * i + cell_w / 2
        out.append("  " + _text(cx, 50, big, size=30, anchor="middle",
                                weight="bold", fill=FG))
        out.append("  " + _text(cx, 74, label, size=13, anchor="middle",
                                fill=MUTED))
        if i:  # a thin divider between cells
            dx = 1 + cell_w * i
            out.append("  " + _rect(dx, 22, 1, height - 44, PANEL_BORDER))
    out.append("</svg>")
    return "\n".join(out)
