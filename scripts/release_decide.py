...
def decide(payload: dict, *, require_ci_green: bool) -> dict:
    ...
    return {
        ...
        "reason": (f"{n} commit(s) since {last_tag or '(no tag)'}, gates green; "
                   f"highest signal = {level} → {nv}"),
...
def main(argv: "list[str] | None" = None) -> int:
    ...
    verdict = decide(payload, require_ci_green=args.require_ci_green)
