...
# A third alias layer: the deeper long-tail — error-message forms ("agent says
# done but git log is empty"), persona/role phrasings (a lead, a PM, a lawyer ask
# the same intent in different words), and host-specific forms not in the host map
# above. Same discipline: every phrasing is a real query a searcher types for an
# intent the page ALREADY answers, none is a new claim, and the flattening below
# dedupes so no string is tracked twice and SKIPS any target page not in the tree
# (so an entry for a page that hasn't landed on this branch yet is simply inert,
# never a dangling citation). This layer takes the captured-query surface past a
# literal 100x of the corpus's birth (docs/325 Phase A, ~9 captured queries).
ALIAS_QUERIES_EXTRA = {
    "docs/answers/how-to-verify-an-ai-agent-actually-did-the-work.md": [
    ...
    "docs/answers/do-ai-coding-agents-lie-about-what-they-shipped.md": [
        "how common is it for coding agents to over-claim",
        "study on AI agents misreporting completed work",
        "agent honesty about shipped work the data",
        "are agent done-claims trustworthy evidence",
        "rate at which agents claim work they didn't do",
    ],
    "docs/answers/process-reward-model-training-data-that-cant-be-gamed.md": [
    ...
    "docs/answers/how-to-add-a-guardrail-to-a-coding-agent-with-no-plugin-system.md": [
        "constrain an agent that has no extension API",
        "minimal trust check for a bare CLI agent",
        "guardrail any agent runtime through exit codes",
        "no hooks no plugins how do I still gate the agent",
        "add a check to an agent the host can't extend",
    ],
    "docs/answers/what-replaced-tokens-burned-as-the-metric-for-ai-agents.md": [
    ...
    "docs/answers/does-the-commit-message-match-what-changed.md": [
        "commit says fix but nothing functional changed",
        "verify a commit subject reflects its actual diff",
        "agent's commit message overstates the change",
        "audit a commit's claim against what it touched",
    ],
    "docs/answers/how-to-verify-a-cited-legal-case-exists.md": [
    ...
    "docs/answers/verify-a-quoted-holding-appears-in-the-opinion.md": [
        "did the AI quote the case accurately",
        "confirm a holding quote is actually in the opinion",
        "check an AI legal quote against the source text",
        "quote attribution check for AI legal output",
    ],
    "docs/answers/make-an-agent-prove-the-work-not-self-certify.md": [
        "agent grades its own completion stop that",
        "demand evidence not a claim from an agent",
        "an agent shouldn't certify its own done",
        "require a witness before an agent calls itself finished",
    ],
    "docs/answers/ai-agent-deleted-my-tests-to-pass-the-build.md": [
    ...
    "docs/answers/refuse-an-agent-action-with-a-structured-reason.md": [
        "typed decline reason for an agent action",
        "machine-checkable refusal instead of a prose error",
        "give an agent a reason code when blocking it",
        "structured no for an agent with a verifiable cause",
    ],
    "docs/answers/catch-allow-empty-shipped-fake-done.md": [
    ...
    "docs/answers/recalled-agent-memory-is-stale-how-to-reverify.md": [
        "agent memory says something no longer true",
        "re-check a saved fact before the agent acts on it",
        "stale recall in an agent how to revalidate",
        "verify agent memory against current repo state",
    ],
    "docs/answers/prove-a-phase-shipped-from-git-history.md": [
    ...
    "docs/answers/lease-based-file-locking-for-parallel-agents.md": [
        "claim a file region before an agent edits it",
        "file lease so agents don't double-book a tree",
        "coordinate agent writes with leases not locks",
        "admission lease for a fleet of coding agents",
    ],
    "docs/answers/verify-what-a-subagent-claims-before-folding.md": [
    ...
    "docs/answers/why-you-cant-trust-a-model-to-judge-its-own-work.md": [
        "self-grading bias in an LLM agent",
        "why an agent scoring itself is unreliable",
        "should an LLM be its own evaluator",
        "the flaw in letting a model judge its own output",
    ],
    "docs/answers/catch-fabricated-figures-in-agent-financial-output.md": [
    ...
    "docs/answers/the-trust-ladder-oracle-judge-human.md": [
        "when to use a hard check vs an LLM judge vs a person",
        "escalate from deterministic check to judge to human",
        "tiered oversight ladder for agents",
        "combine oracle judge and human review",
    ],
    "docs/answers/refusal-as-a-first-class-primitive-for-agents.md": [
    ...
    "docs/answers/stop-an-agent-editing-ci-to-skip-tests.md": [
        "agent turned off a CI check to pass",
        "prevent an agent weakening the build pipeline",
        "agent edited the workflow to skip tests block it",
        "stop an agent disabling failing CI steps",
    ],
    "docs/answers/block-an-out-of-lane-file-write-at-pretooluse.md": [
    ...
    "docs/answers/open-source-ai-legal-citation-checker.md": [
        "free citation verifier for legal AI",
        "self-hosted AI case-law checker open source",
        "open source tool to validate AI legal citations",
        "no-cost alternative to paid citation checkers",
    ],
    "docs/answers/scavenge-a-stalled-lease-without-killing-a-live-one.md": [
    ...
    "docs/answers/two-claude-code-agents-on-one-branch.md": [
        "two Claude Code sessions on one branch colliding",
        "parallel Claude Code agents clobber each other",
        "coordinate multiple Claude Code agents on a repo",
        "two agents same branch keeps breaking my work",
    ],
    "docs/answers/make-agent-done-mean-a-checkable-effect.md": [
        "agent done as an observable effect not a sentence",
        "tie completion to a real verifiable change",
        "what should done mean for an autonomous agent",
        "make done a fact the agent can't fake",
    ],
    "docs/answers/can-i-trust-a-coding-agents-pull-request.md": [
    ...
    "docs/answers/enforce-that-an-agent-ran-the-tests-it-claims.md": [
        "did the agent actually run the test suite",
        "require proof the agent executed its tests",
        "agent claims it tested verify the run",
        "make an agent show it ran the tests",
    ],
    "docs/answers/catch-an-agent-that-fakes-tool-calls-or-output.md": [
    ...
    "docs/answers/prevent-context-poisoning-from-an-agents-own-outputs.md": [
        "agent contaminates its context with bad output",
        "self-poisoning feedback loop in an agent",
        "stop an agent recycling its own hallucinations",
        "prevent an agent's prior mistakes from compounding",
    ],
    "docs/answers/multi-agent-coordination-without-a-central-orchestrator.md": [
        "decentralized coordination for an agent fleet",
        "agents coordinate via contracts not a controller",
        "no orchestrator how do agents avoid collisions",
        "orchestrator-free multi-agent setup",
    ],
    "docs/answers/builder-validator-chain-separate-generator-from-evaluator.md": [
    ...
    "docs/answers/verify-an-llm-didnt-hallucinate-a-function-or-api.md": [
        "agent called a function that doesn't exist",
        "detect a hallucinated API in generated code",
        "check the library method the agent used is real",
        "LLM invented a function verify it exists",
    ],
    "docs/answers/hidden-test-split-to-stop-agents-overfitting.md": [
        "held-out tests so an agent can't overfit",
    ...
    "docs/answers/make-any-agent-skill-verify-its-own-work.md": [
        "add self-verification to an existing skill",
        "ground a skill's claim on a real check",
        "turn a belief-based skill into a verified one",
        "give any agent skill a witness",
    ],
    "docs/answers/catch-fabricated-legal-citations-in-my-ai-agent.md": [
    ...
    "docs/answers/ai-generated-tests-that-pass-but-test-nothing.md": [
        ...
        "agent tests are green but assert nothing",
        "vacuous passing tests from an AI",
        "AI tests always pass and verify nothing",
    ],
    "docs/answers/ai-mocks-everything-tests-are-useless.md": [
    ...
    "docs/answers/mutation-testing-vs-test-witness-for-ai-tests.md": [
        "is mutation testing enough for fake AI tests",
        "test-witness vs mutation testing for agent tests",
        "prove AI tests catch real bugs",
        "best way to validate AI test assertions",
    ],
    "docs/answers/make-ai-write-tests-that-actually-assert.md": [
    ...
    "docs/answers/stop-re-reviewing-code-the-machine-already-verified.md": [
        "skip review on code the gate already verified",
        "spend review only on the unverified residual",
        "save reviewer time on machine-checked changes",
        "route human review to what the kernel couldn't witness",
    ],
}

# A fourth layer on the highest-traffic intents — the same questions in a
# searcher's plainest words and the imperative/"how do I" splits. These pages each
# answer far more than a dozen real phrasings; this captures more of them. Same
# discipline (real query, no new claim, deduped, page must exist).
ALIAS_QUERIES_EXTRA2 = {
    "docs/answers/how-to-verify-an-ai-agent-actually-did-the-work.md": [
    ...
    "docs/answers/do-ai-coding-agents-lie-about-what-they-shipped.md": [
        "can I believe what a coding agent says it did",
        "AI agents exaggerating their work",
        "agents claiming more than they shipped",
        "how truthful are AI coding agents about output",
    ],
    "docs/answers/stop-ai-making-fake-tests.md": [
    ...
    "docs/answers/make-an-agent-prove-the-work-not-self-certify.md": [
        "stop an agent marking its own work done",
        "force an agent to back up its claims",
        "no self-certified done for agents",
        "agent must prove not just assert completion",
    ],
    "docs/answers/catch-fabricated-legal-citations-in-my-ai-agent.md": [
    ...
    "docs/answers/refuse-an-agent-action-with-a-structured-reason.md": [
        "block an agent action with a real reason",
        "structured reasons for declining agent actions",
        "deny an agent and say why in a checkable way",
        "reason codes for blocked agent actions",
    ],
    "docs/answers/trust-substrate-for-a-fleet-of-autonomous-agents.md": [
...
# Flatten all alias maps into the same (query, page) shape as the curated core.
# Order is deterministic; the core list stays first. A phrasing already present is
# skipped (first occurrence authoritative -> unique strings, the uniqueness guard).
# An alias whose target page is not in the tree is skipped, so the list never
# carries a dangling citation (robust to a page that lands on a branch later).
_seen_queries = {q for q, _ in ARRIVAL_QUERIES}
for _amap in (ALIAS_QUERIES, ALIAS_QUERIES_HOST, ALIAS_QUERIES_EXTRA, ALIAS_QUERIES_EXTRA2):
    for _page, _aliases in _amap.items():
        if not (REPO / _page).is_file():
            continue
        for _q in _aliases:
