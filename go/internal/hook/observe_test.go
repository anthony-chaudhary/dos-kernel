...
func TestRecordObservationWritesSchemaTaggedLine(t *testing.T) {
	...
	for _, k := range []string{"verb", "outcome", "rung", "reason_class", "dialect", "tree_known", "latency_ms", "call_id", "session_id", "workspace", "profile", "phase_state"} {
		if _, ok := o[k]; !ok {
...
func TestRecordObservationOmitsUnsetFields(t *testing.T) {
	...
	for _, k := range []string{"rung", "reason_class", "dialect", "verify_source", "blocked_plan", "claims_seen", "stream_state", "call_id", "session_id", "workspace", "profile", "phase_state"} {
		if _, ok := o[k]; ok {
