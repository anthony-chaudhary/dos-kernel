...
| AV6 | D8 | **Running the suite is effect-free on tracked files.** A session-scoped conftest fixture snapshots the set of modified tracked paths (`git status --porcelain`, tracked entries only) before the session, records repo paths this pytest process opens for write, and re-derives status at session end; assert no *new* entries that this session wrote. Delta-of-sets plus self-write attribution, not absolute cleanliness — the hot tree is legitimately dirty with concurrent work; the suite must only add nothing itself. |
