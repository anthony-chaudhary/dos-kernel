...
| `loop-hero.png` | **High-res raster backup** of the hero's resolved (reduced-motion) frame — 3600×1920. A general-purpose poster for slide decks and the announce docs on surfaces that don't render SVG. (For the **share/Open Graph card** specifically, use the purpose-built `social-card.png` above — it's sized 1280×640 for the social slot; this hero poster is the wrong ratio and over GitHub's 1 MB cap.) GitHub embeds the SVG; this is the fallback poster. | Rendered from `loop-hero.svg` at 3× DPR (the convert recipe below). |
