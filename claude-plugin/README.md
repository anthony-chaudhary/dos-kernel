...
|---|---|---|
| **Hooks** | [`hooks/hooks.json`](hooks/hooks.json) | `PreToolUse` → `dos hook pretool` (DENY a structurally-refused call before it runs) · `Stop` → `dos hook stop` (refuse to stop on an unverified claim). The advisory `PostToolUse` checkpoint is intentionally omitted: Codex renders every declared hook in the foreground, and stalled-stream recovery remains on the Stop/watchdog path. Served by the bundled native `dos-hook` binary (`bin/`) in ~10 ms, with a Python fallback. |
